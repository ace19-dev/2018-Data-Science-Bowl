...
class BestCheckpointSaver(object):
    ...
    framework.
    """
    def __init__(self, title, save_dir, num_to_keep=1, maximize=True, saver=None):
        ...
        self._save_path = os.path.join(save_dir, title)
    ...
    def handle(self, value, sess, global_step_tensor, global_step_value):
        ...
        current_ckpt = '{}-{}'.format(self._title, global_step_value)
        value = float(value)
        if not os.path.exists(self.best_checkpoints_file):
            self._save_best_checkpoints_file({current_ckpt: value})
            self._saver.save(sess, self._save_path, global_step=global_step_value)
            return
